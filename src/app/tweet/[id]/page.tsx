...
export default async function Page({
  ...
  return (
    ...
    <section className="flex w-full max-w-[600px] h-full min-h-screen flex-col border-x-2 border-x-gray-700/60 p-4">
       <Tweet tweet={tweet[0]} user={session ? session.user : undefined} fromReplies={true}/>
       <div>{replies.map(reply => <Reply key={reply.id} reply={reply} userId={session ? session.user.id : ''}/>)}</div>
    </section>
  )
}
